...
// ReSharper restore CheckNamespace
{
    ...
    {
        #region Variables
        public readonly DynamicQueryDatabase Database;
        #endregion
        ...
        public QueryBuilder()
        {
            ...
            //var logic = new DynamicQueryLogic();
        ...
        public List<TableRoute> CalculateRoute(string tableName)
        {
            ...
            var shortestPaths = Database.Tables.Select(table => table.Name == tableName 
                                                        ? new TableRoute { Table = table.Name, Depth = 0 } 
                                                        : new TableRoute { Table = table.Name, Depth = int.MaxValue }
                                                       ).ToList();

            while (handledRoute.Count != Database.Tables.Count)
            {
                ...
                var selectedConnections = from c in Database.Associations
                ...
                selectedConnections = from c in Database.Associations
